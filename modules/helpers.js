...
async function getConversationName(req, conv) {
  const userId = req.session.user._id;
  const convUserIds = conv.users.filter(id => id.toString() !== userId); // filter out the user making the request // to be changed to: conv.users.map(usr => usr.refId).filter(id => id.toString() !== userId)
...
async function createSendConvObj(req, targetConv){
  ...
  if (userConversations) {
    const targetConversation = targetConv;
    ...
    sendConvObj.targetConversation = targetConversation;
    sendConvObj.conversations = sendConversations;
  } else {
    sendConvObj.targetConversation = targetConv;
